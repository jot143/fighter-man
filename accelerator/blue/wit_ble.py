...
def main():
    ...
    if arg.mac == '':
        ...
        # Retry logic for scanning (handles Bluetooth "cold start" issues)
        max_retries = 3
        devices = None
        for retry in range(max_retries):
            try:
                ...
                if retry < max_retries - 1:
                    print(ANSI_YELLOW + "Scan failed (Bluetooth not ready), retrying... ({}/{})".format(retry + 1, max_retries) + ANSI_OFF)
                    time.sleep(0.5)
                    # Recreate scanner for next attempt
                    scanner = btle.Scanner(arg.hci).withDelegate(ScanPrint(arg))
                else:
                    print(ANSI_RED + "Scan failed after {} retries".format(max_retries) + ANSI_OFF)
                    raise

        if devices is None:
            print(ANSI_RED + "Scan failed, no devices found" + ANSI_OFF)
            return

        connectaddr = None
        connectaddrtype = None
        for d in devices:
            ...
            for (sdid, desc, val) in d.getScanData():
                if arg.name == val:
                    ...
                    connectaddr = d.addr
                    connectaddrtype = d.addrType
                    break
    else:
        connectaddr = arg.mac
        connectaddrtype = btle.ADDR_TYPE_PUBLIC
    if connectaddr != None:
        p = btle.Peripheral()
        p.setDelegate(MyDelegate())
        try:
            p.connect(connectaddr, connectaddrtype)
            p.setMTU(247)
            # # get all services
